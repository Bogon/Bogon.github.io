...
实际上，您已经看到了模式匹配的实际应用。 在第4章“高级控制流”中，使用switch语句在不同情况下匹配数字和字符串。 这是一个简单的示例，但是在该主题上还有很多值得探索的地方。

您将深入了解其基本机制，并进一步了解Swift编译器如何解释您键入的代码。

Swift是一种多范式语言，可让您构建功能全面，可用于生产的，面向对象的软件。 Swift的设计师从诸如Haskell和Erlang的更多功能样式语言中借鉴了一些技巧。

模式匹配是这些功能语言的基础。 它使您不必键入更长且可读性更差的语句来评估条件。
...
以下各节将向您展示如何以及何时使用代码中的模式。

# 模式介绍

模式提供规则以匹配值。 您可以在切换用例以及if，while，guard和for语句中使用模式。 您还可以在变量和常量声明中使用模式。

信不信由你，您已经看到了另一个带有协调元组声明的模式示例。 通过用括号之间的逗号分隔值（如（x，y，z））来构造元组。 编译器将理解该模式是指三个值的元组：x，y和z。 元组具有复合值的结构。

单个值也具有结构。 数字42是单个值，从本质上来说是可识别的。

**模式定义值的结构。 模式匹配使您可以相互检查值。**

> 注意：值的结构不引用结构类型。 尽管它们使用相同的词，但它们是不同的概念。 可能是语言匮乏的症状！

# 基本模式匹配

在本部分中，您将看到模式匹配的一些常见用法。

## If 和 guard

到目前为止，您都使用了普通的old if和guard语句。 您可以使用大小写条件将它们转换为模式匹配语句。 下面的示例显示了如何使用带有条件条件的if语句：
...
在一种情况下，先写模式，后跟等号=，然后写要与模式匹配的值。 如果您希望匹配单个模式，则if语句和保护语句效果最佳。

## Switch

如果您想匹配多个模式，则switch语句是您最好的朋友。 您可以像这样重写processPoint（）：
...
switch语句比if语句还具有优势，因为它进行了详尽的检查。 编译器保证您在switch语句结束之前已经检查了所有可能的值。

同样，请记住，switch语句将以匹配的第一种情况条件退出。 这就是为什么您将midRange条件放在第二位的原因。 即使midRange条件与closeRange值匹配，除非前一个条件失败，否则不会进行评估。 默认情况是包罗万象。 如果在所有其他情况下都没有匹配项，则将执行默认情况。
...
在此示例中，该数组提供了学校教室的工作组大小列表。 循环的实现仅针对与值1匹配的数组中的元素运行。由于鼓励班级中的学生而不是他们自己进行团队合作，因此您可以隔离尚未找到伙伴的人。

# 模式

既然您已经看到了一些模式匹配的基本示例，那么让我们来谈谈可以匹配的模式。

## 通配符模式

重新查看在本章开始时看到的示例，在该示例中，您要检查（x，y，z）元组坐标的值是否在x轴上：
...
价值绑定模式听起来比实际中复杂得多。 您只需使用var或让我们在匹配模式时声明变量或常量。

然后，您可以在执行块中使用变量或常量的值：
...
标识符模式比值绑定模式更为直接。 标识符模式是常量名或变量名本身。 在上面的示例中，这就是模式中的x。 您告诉编译器，“当找到（something，0，0）的值时，将某物分配给x。”

由于标识符模式是值绑定模式的子模式，因此该描述与您之前看到的内容交织在一起。

## 元组模式

您已经使用了另一种奖金模式-您知道吗？ 元组不仅仅是括号之间的一系列逗号分隔值：它实际上是逗号分隔的模式。 在示例元组模式（某物，0，0）中，内部模式是（标识符，表达式，表达式）。

您将在本章末了解表情模式。 目前，重要的一点是，元组模式将许多模式组合为一个，并可以帮助您编写简洁的代码。

## 枚举案例模式

在第15章“枚举”中，您了解了如何匹配枚举的成员值：
...
在该代码中，.animal的关联值绑定到名为legs的常数。 您可以在该条件的执行块内的打印调用中引用支腿常量。

关联值被锁定在枚举值中，直到您使用值绑定模式来提取它们为止.

## 可选模式

说到可选，还有一个可选模式。 可选模式包括一个标识符模式，后跟一个问号。 您可以在使用枚举用例模式的相同位置使用此模式。
...
通过在大小写条件下使用is运算符，可以检查实例是否为特定类型。 何时使用此功能的一个示例是通过JSON导出进行解析。 如果您不熟悉，JSON基本上是一个包含所有不同类型的数组，您可以在Swift中将其写为[Any]。 Web API和网站开发人员大量使用JSON。

因此，从网络API解析数据时，您需要检查每个值是否属于特定类型：
...
使用此代码，您会发现其中一个元素是String类型的。 但是您无法在实现中访问该String的值。 这就是救援的下一个模式。

## “As”类型转换模式

as运算符将is类型转换模式与值绑定模式组合在一起。 扩展上面的示例，您可以编写这样的情况：
...
您已经完成了以上所有模式！ 到目前为止，您在本章中学到的知识将带您成为一名开发人员。 在接下来的部分中，您将学习一些修饰符技巧，使您可以进一步整合代码。

## Qualifying with where

您可以通过在线检查一元条件来指定where条件以进一步过滤匹配项。 在第4章“高级控制流”中，您看到了这样的示例：
...
您可以通过枚举以更复杂的方式利用其中的位置。 假设您正在编写一个游戏，想要保存每个级别的玩家进度：
...
您在第4章“高级控制流”中了解到的另一件事是如何在单例情况下匹配多个模式。 这是一个与您之前看到的类似的示例：
...
在这里，您会看到几种在每种情况下都匹配的标识符模式。 您可以在每个逗号后面的模式中使用在前面的模式中绑定的常量和变量。 这是对可爱动物测试的改进：
...
将所有这些if语句嵌套在另一个语句中被称为厄运金字塔。 相反，您可以在连续逗号后立即使用展开和绑定的值：
...
因此，现在您可以看到，模式匹配可以与简单的逻辑条件和可选的绑定结合在一个if语句中。 您的代码看起来已经很优雅了！

## 自定义元组

在本章中，您了解了元组模式如何与三维坐标（x，y，z）匹配。 准备好匹配时，您可以创建一个即时元组表达式。
...
在这里，您将名称和年龄常数组合成一个元组，并将它们一起评估。

另一个这样的示例涉及带有用户名和密码字段的登录表单。 用户因不完整填写字段然后单击“提交”而臭名昭著。 在这些情况下，您想向用户显示一条特定的错误消息，指示缺少的字段，如下所示：
...
该代码执行其动作三遍。 下划线_表示您无需使用序列中的每个值。 如果您发现自己需要重复操作，这是编写代码的一种干净方法。
...
在此代码中，您检查以确保用户具有值。 您使用下划线表示目前，您不在乎其包含的值。

即使您可以做某事，也并不意味着您应该这样做。 在不关心值的情况下验证可选值的最佳方法是：
...
您可以将此代码编写为一系列if语句。 使用switch语句时，很明显每个条件都是一个案例。 请注意，每种情况都使用带有限定where子句的下划线。

## 表达式模式

到目前为止，您已经掌握了所有的模式匹配技能，终于可以开始学习引擎盖下的内容了。表达式模式很简单，但是，它是如此强大。

在本章的开头，您看到了示例元组模式（x，0、0）。您了解到，在内部，元组是逗号分隔的模式列表。您还了解到x是标识符模式，而0则是表达式模式的示例。因此，该元组的内部模式是（标识符，表达式，表达式）。

表达式模式将值与模式匹配运算符〜=进行比较。当比较返回true时，匹配成功。如果值是同一类型，则公共==等号运算符将执行比较。在第16章“协议”中，您学习了如何为自己的命名类型实现Equatable和==。

当值的类型不同或类型未实现Equatable协议时，将使用〜=模式匹配运算符。

例如，编译器使用〜=运算符检查整数值是否落在范围内。该范围肯定不是整数，因此编译器不能使用==运算符。但是，您可以概念化检查Int是否在范围内的想法。这就是〜=模式匹配运算符出现的地方：
...
您可以重载〜=运算符以提供您自己的自定义表达式匹配行为。 您将在数组和整数之间实现模式匹配，以检查整数是否是数组的元素。 值2应该匹配模式[0，1，2，3]。 使用标准库，您将在以下代码上看到错误：
...
当然，您可以检查整数是否在数组中，如下所示：

``` Swift
let isInList = list.contains(integer) // true
```

但是最好使用模式匹配，以便您可以在switch语句中检查是否匹配。 您可以使用以下代码实现缺少的模式匹配器：
...
现在，模式匹配运算符已被重载，表达式您之前看到的模式现在可以正确匹配且没有错误。
...
您现在是匹配忍者的模式！ 掌握了模式之后，您就可以编写清晰，简洁，易读的代码了。

# 关键点

+ 模式代表值的结构。

+ 模式匹配可以帮助您编写比其他逻辑条件更可读的代码。
