...
在本篇中，你将学习RxSwift中不同类型的元素，了解如何处理每个元素，以及为什么要根据一些常见用例选择一个元素。 你还将学习中继，中继是围绕元素的包装。
...
+ PublishRelay和BehaviorRelay：这些包装它们的相对元素，但仅接受.next事件。你根本无法在中继上添加.completed或.error事件，因此它们非常适合非终止序列。
...
如前所述，中继器在保留元素的同时保持其重放行为。 与其他元素（以及通常的可观察对象）不同，你可以使用accept（_ :)方法将值添加到中继上。 换句话说，你不使用onNext（_ :)。 这是由于中继只能接受值，并且你不能在它们上添加.error或.completed事件。
